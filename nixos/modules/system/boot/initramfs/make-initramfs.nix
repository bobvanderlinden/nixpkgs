let
  ...
  # from it.
  compressors = import ./compressor-meta.nix;
...
, name ? "initrd"
, find-dependencies ? pkgs.callPackage ./find-dependencies { }
...
, compressor ? "gzip"
, _compressorFunction ? if lib.isFunction compressor
  then compressor
...
, compressorArgs ? null
, _compressorArgsReal ? if compressorArgs == null
      ...
      linkFarmEntries = map
        ({ object
         ...
         ,
          ...
          name = stripRoot symlink;
          path = object;
        })
        symlinks;
    in
    linkFarm "root" linkFarmEntries;

  nativeBuildInputs = [ find-dependencies patchelf glibc cpio strip-nondeterminism ];
} ''
  mkdir "$out"

  set -o pipefail
  cd $root && (find-dependencies < "$contentsPath" && find * .[^.*]) \
